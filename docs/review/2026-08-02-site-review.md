...
> **Implementation status (added 2026-08-03).** The remediation plan in section 6 was implemented on this branch in four commits (batches 1–4), excluding SEC-01 and SEC-02 at the owner's request. All severity-bearing findings were addressed except three that need owner input: **PRIV-07** (international-transfer sentence — drafted as a template comment in `src/privacy/index.njk` pending confirmation of the hosted backend's Azure region, which is not stated in any public doc), **POL-02** (the footer's "source-available / All Rights Reserved" phrase is unlinked because no LICENSE file exists in `eslutz/PumpSync` to link — licensing terms are an owner decision), and **PRIV-12** (the wiki's verbatim policy copies must be edited in the wiki repository, which this session cannot push to — replace the duplicated sentences in `App-Store-and-Privacy.md` with links to `/privacy/`). Items marked *Discussion* were deliberately left as owner decisions, with two exceptions folded in because the plan's batches included them: the caregiver sentence (AGE-03) and intent comments on the editorial-policy tests (TEST-07). Two verify-on-device items remain: the Apple Health revocation path now matches the app's own instructions (COPY-02) but should be confirmed on a current release, and the deletion page's new 30-day/confirmation commitment (DEL-03) plus the best-effort no-ID fallback (DEL-01) are commitments the owner should confirm they can honor. The findings below are otherwise left as written on 2026-08-02 and describe the pre-fix state.
